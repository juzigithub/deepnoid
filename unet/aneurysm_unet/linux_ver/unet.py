...
class Model:
    ...
    def __init__(self, loss='dice', upsampling_mode='resize', normalization_mode='Batch', groupnormalization_n=2, downsampling_option='neighbor',
        ...
        if self.channel_mode == 'NHWC':
            ...
            self.X = tf.placeholder(tf.float32, [None, n_channel, img_size, img_size], name='X')
            self.Y = tf.placeholder(tf.float32, [None, n_class, img_size, img_size], name='Y')


##############################################################################################################
        self.dataset = tf.data.Dataset.from_tensor_slices((self.X, self.Y)).shuffle(buffer_size=3000)
