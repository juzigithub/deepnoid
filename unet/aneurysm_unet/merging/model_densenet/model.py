...
###############################
        ...
        with tf.variable_scope('down'):
            ...
            for i in range(cfg.DEPTH):
                pool_size //= 2
                self.down_conv[i] = utils.dense_block_v1('dense' + str(i), inputs, cfg.GROUP_N, self.drop_rate, self.training, 1)
                print('down_conv', self.down_conv[i])
                self.channel_n[i] = self.down_conv[i].get_shape().as_list()


                self.down_pool[i] = utils.select_downsampling(str(i) + '_downsampling', self.down_conv[i], self.down_pool[i], self.channel_n[i] * 2, pool_size, cfg.DOWNSAMPLING_TYPE)
        ...
        with tf.variable_scope('up'):
            ...
            for i in reversed(range(cfg.DEPTH)):
                channel_n //= 2
                pool_size *= 2
                inputs = utils.unet_up_block(inputs, self.down_conv, self.up_conv, self.up_pool, self.channel_n[i], pool_size, cfg.GROUP_N, self.training, i)
