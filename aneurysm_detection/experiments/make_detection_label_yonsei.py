...
for i_dir, l_dir in zip(input_dir_list, label_dir_list):
    ...
    for label_path in label_path_list:
        ...
        labels = np.transpose(labels, (2, 1, 0)).astype(np.uint8)
        # labels = np.flip(np.flip(labels, axis=1), axis=2)
        # print(labels.shape)
        # labels = labels.copy() * 255
        # print('labels', labels)
        for idx, img in enumerate(labels):
            ...
            label_list = []
            # img = img.copy() * 255 ### .copy() why?

            for label_idx in range(len(label_areas)):
                # label_idx += 1

                x1, y1 = stats[label_idx + 1, cv2.CC_STAT_LEFT], stats[label_idx + 1, cv2.CC_STAT_TOP]
                x2, y2 = x1 + stats[label_idx + 1, cv2.CC_STAT_WIDTH], y1 + stats[label_idx + 1, cv2.CC_STAT_HEIGHT]
                label_list.append([y1 / height, x1 / width, y2 / height, x2 / width])
                # print(y1-5, x1-5, y2+5, x2+5)
                # cv2.rectangle(img, (x1-10, y1-10), (x2+10, y2+10), (255, 255, 255), 1)
            ...
            if label_list != [] :
                ...
                # img *= 255
                # print(np.max(img))
                # img = utils.masking_rgb(img)
                # cv2.imwrite(path + '/' + 'detection_' + fn + '.png', img)
                # if os.path.exists(os.path.join(SAVE_PATH, l_dir, 'detection_' + fn + '.png')):
                #     print(np.max(cv2.imread(os.path.join(SAVE_PATH, l_dir, 'detection_' + fn + '.png'), cv2.IMREAD_GRAYSCALE)))
                    # print(np.shape(cv2.imread(os.path.join(SAVE_PATH, l_dir, 'detection_' + fn + '.png'), cv2.IMREAD_GRAYSCALE)))

                    # img += utils.masking_rgb(cv2.imread(os.path.join(SAVE_PATH, l_dir, 'detection_' + fn + '.png'), cv2.IMREAD_GRAYSCALE))
                    # prev_img = cv2.imread(os.path.join(SAVE_PATH, l_dir, 'detection_' + fn + '.png'), cv2.IMREAD_GRAYSCALE)
                    # img += (prev_img / np.max(prev_img)).astype(np.uint8)
                # cv2.imwrite(os.path.join(SAVE_PATH, l_dir, 'detection_' + fn + '.png'), img)
