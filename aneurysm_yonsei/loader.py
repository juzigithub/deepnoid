...
def save_resized_dcm_as_npy(data_path, save_path, filename):
    ...
    for x, y in zip(x_path_list, y_path_list):
        ...
        x_img = cv2.resize(x_img, (IMG_SIZE, IMG_SIZE), interpolation=cv2.INTER_AREA)

        x_img = (x_img - np.mean(x_img)) / np.max(x_img)
        x_img = np.expand_dims(x_img, axis=0)

        y_img = cv2.imread(y, cv2.IMREAD_GRAYSCALE)
        y_img = cv2.resize(y_img, (IMG_SIZE, IMG_SIZE), interpolation=cv2.INTER_AREA)

        y_img_fg = cv2.threshold(y_img, 30, 1, cv2.THRESH_BINARY)[1]
        y_img_fg = y_img_fg.reshape((1, IMG_SIZE, IMG_SIZE, 1))
        y_img_bg = cv2.threshold(y_img, 30, 1, cv2.THRESH_BINARY_INV)[1]
        y_img_bg = y_img_bg.reshape((1, IMG_SIZE, IMG_SIZE, 1))
