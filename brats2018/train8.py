    ...
    def train(self):
        ...
        with tf.Session() as sess:
            ...
            for variable in tf.global_variables():
                if 'rescale' not in str(variable):
                    sess.run(tf.variables_initializer([variable]))
                    print('>>> initialized ', variable)
                else:
                    self.uninitialized_vars.append(variable)
            print('uninitialized_vars : ', self.uninitialized_vars)
            ...
            for epoch in range(cfg.EPOCHS):
                ...
                for batch in tl.iterate.minibatches(inputs=val_X, targets=val_Y,
                    ...
                    #
                    #         ori = np.transpose(batch_x, [-1, 0, 1, 2]) / 255
                ...
                et_total_result_list.append(et_one_epoch_mean)
                tc_total_result_list.append(tc_one_epoch_mean)
                wt_total_result_list.append(wt_one_epoch_mean)
